...
func TestTOTP(t *testing.T) {
	...
	RunTestWithTempBboltMfaStorage(t, "testStoreGetCodes", testStoreGetCodes)
	RunTestWithTempBboltMfaStorage(t, "testStoreGetCodes", testDeleteTOTPCodeByCode)
...
func testStoreGetCodes(t *testing.T, storage TOTPSecretStorage) {
	totp := &TOTPImpl{s: storage}
	...
	for _, testTOTP := range testTOTPs[:3] {
		err := storage.StoreTOTPSecret(testTOTP.TOTPStored)
...
func testDeleteTOTPCodeByCode(t *testing.T, storage TOTPSecretStorage) {
	totp := &TOTPImpl{s: storage}

	err := storage.StoreTOTPSecret(testTOTPs[0].TOTPStored)
	require.NoError(t, err)
...
func testDeleteDuplicateIssuerUser(t *testing.T, storage TOTPSecretStorage) {
	totp := &TOTPImpl{s: storage}
	...
	currentTimestamp := uint64(time.Now().Unix())
	err = totp.RemoveCodeByTOTPCode(codeToDelete, currentTimestamp)
	require.NoError(t, err)
...
func testInvalidAlgorithm(t *testing.T, storage TOTPSecretStorage) {
	// Store a secret with an invalid algorithm
	invalidTOTP := testTOTPs[0]
	invalidTOTP.TOTPStored.Algorithm = "MD5"
	err := storage.StoreTOTPSecret(invalidTOTP.TOTPStored)
	require.Error(t, err)
}

func testInvalidPeriod(t *testing.T, storage TOTPSecretStorage) {
